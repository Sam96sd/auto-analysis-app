...
## 📡 مصادر البيانات واستهلاك الحد المجاني

1. **Twelve Data** (المصدر الأساسي، سعر فوري) — يحتاج `TWELVE_DATA_API_KEY` مجاني.
   - **طلبان فقط لكل أصل:** شموع 15 دقيقة (5000 شمعة تُشتق منها H1 و H4) + الشموع اليومية.
   - **ذاكرة مشتركة بين كل الزوار:** 15 دقيقة تتحدث كل دقيقتين، واليومي كل ساعة. عدد الزوار لا يزيد الاستهلاك.
   - الحد الأقصى ≈ 31 طلباً في الساعة لكل أصل مفتوح (الحد المجاني 800 يومياً و8 في الدقيقة).
2. **Binance** (احتياطي مجاني بلا مفتاح) — يعمل تلقائياً عند نفاد حد Twelve Data: البيتكوين (BTC/USDT)، الذهب عبر PAXG/USDT (تقريبي)، اليورو عبر EUR/USDT (تقريبي).
3. **Yahoo Finance** — احتياطي أخير (مفيد للفضة).
4. إذا فشلت كل المصادر تُعرض آخر بيانات ناجحة (حتى 15 دقيقة) مع تنبيه، ثم رسالة خطأ واضحة. البيانات التجريبية تعمل أثناء التطوير المحلي فقط.

> الملف `vercel.json` يشغّل الخادم في فرانكفورت (`fra1`) لأن Binance يحجب الخوادم الأمريكية، وهي أيضاً أقرب لمستخدمي الخليج.
> ملاحظة: الخطة المجانية في Twelve Data مخصصة للاستخدام الداخلي (non-display)، راجع شروطهم قبل الإطلاق للعامة.

## 🔒 حماية الموقع (دخول بحساب Google لإيميلات محددة)

- كل صفحات الموقع وواجهة البيانات محمية بـ `proxy.ts`.
- الزائر يسجل الدخول بحساب Google، ويُسمح له فقط إذا كان إيميله في **`allowed-emails.ts`**.
- لإضافة أو حذف عضو: عدّل `allowed-emails.ts` (من GitHub مباشرة بأيقونة القلم) ← Commit ← يتحدث الموقع خلال دقيقة. من يُحذف يُمنع فوراً حتى لو كان مسجلاً.
- جلسة الدخول صالحة 14 يوماً ثم يُطلب الدخول مجدداً بضغطة واحدة.
- متغيرات البيئة المطلوبة: `AUTH_SECRET`, `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET` (انظر `.env.example`).
- أثناء التطوير المحلي بدون هذه المتغيرات يبقى الموقع مفتوحاً، وفي الإنتاج يُغلق برسالة واضحة.
- رابط العودة الذي يُسجَّل في Google: `https://YOUR-DOMAIN/api/auth/callback`
